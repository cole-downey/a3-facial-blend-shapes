...
public:
	...
	vector<int> blendBinds; // bind mesh # corresponding to each blendshape
};
...
void init() {
	...
	for (const auto& mesh : dataInput.meshData) {
		...
		for (int i = 2; i < mesh.size(); i++) {
			shape->addBlendshape(DATA_DIR + mesh[i]);
		}
	}
	...
	prog->addAttribute("b1Pos");
	prog->addAttribute("b1Nor");
	prog->addUniform("b1Weight");
	prog->addAttribute("b2Pos");
	prog->addAttribute("b2Nor");
	prog->addUniform("b2Weight");
	prog->addUniform("P");
	prog->addUniform("MV");
	prog->addUniform("ka");
	prog->addUniform("ks");
...
void loadDataInputFile() {
	...
	while (1) {
		...
		if (key.compare("TEXTURE") == 0) {
			...
			ss >> value;
			ss >> bindMesh;
			dataInput.meshData.at(bindMesh).push_back(value);
		} else {
